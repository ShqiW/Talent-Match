...
def create_app():
    ...
    candidate_processor = CandidateProcessor(embedding_processor)
    recommendation_engine = RecommendationEngine(embedding_processor)
    
    # 初始化服务
    candidate_service = CandidateService(candidate_processor)
    recommendation_service = RecommendationService(recommendation_engine, candidate_processor)
    
    # 注册路由
    app.register_blueprint(create_health_routes())
    app.register_blueprint(create_candidate_routes(candidate_service, app.config))
    app.register_blueprint(create_recommendation_routes(recommendation_service, candidate_service, app.config))
